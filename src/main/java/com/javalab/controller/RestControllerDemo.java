 ...
 */
@RestController // @Controller + @ResponseBody 결함된 컨트럴로
@Slf4j
public class RestControllerDemo {
   ...
   public  MemberVo action4(@PathVariable("name") String name,
      ...
      return member;  //JSON Type문자열로 변환(@ResponseBody + Jackson-databind)
   }
   /*
    * @RestController의 모든 메소드는 @ResponseBody가 자동으로 붙음
    *  - @ResponseBody : 요청한 페이지의 바디에 출력
    *  - @RequestBody MemberVo : JSP에서 ajax형태로 요청한 값들을 커맨드 객체에 자동으로 바인딩
    * */
