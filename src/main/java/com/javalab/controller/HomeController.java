...
public class HomeController {
    ...
    * 응답 페이지가 생략되면 호출된 url formRest2가 변환되어
   ...
   public void formRest2(@ModelAttribute("name") String name) {
      //return "formRest2"; 생략해도 응답 페이지의 이름은 위에서 @RequestMapping에 전달한 value 값으로 설정됨
   }
   
   /*
    * formRest.jsp에서 ajax 형태로 호출됨.
    * [@PathVariable] 형태로 값을 전달 받는 방법
    *  - 경로명?name="홍길동"&grade="3" 과 같은 형태
    * [@ResponseBody]
    *  - 호출한 곳으로 응답할 때 "메시지 바디"에 담아서
    *  	 전달해주면 받는 jsp페이지 Body에 그대로 전달됨.
    *  	 자바스크립트에서 그걸 가공하여 사용함.
    *  - <jacson-databind> 디펜던시(라이브러리)가 필요함.
